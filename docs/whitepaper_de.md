...
Künstliche Intelligenzen, die als persönliche Assistenten oder Geschäftsagenten Aufgaben autonom ausführen, gewinnen rasant an Popularität. Projekte wie OpenClaw (früher ClawdBot oder MoltBot) zeigen, wie leistungsfähig solche Agenten sein können. Sie verbinden sich gleichzeitig mit Messengern, E‑Mail‑Systemen, Cloud‑Diensten, Git‑Repositorys und Betriebssystemwerkzeugen. Dieser Komfort geht jedoch mit gravierenden Sicherheitsrisiken einher. Untersuchungen zeigen, dass OpenClaw umfangreiche Berechtigungen benötigt – Dateisystemzugriff, Bash‑Ausführungsrechte, API‑Schlüssel und Netzwerkzugriffe – wodurch die Komplexität und damit die Angriffsfläche steigt. Nutzer installieren den Assistenten häufig mit weitreichendem Zugang zu E‑Mails, Dateien, Kalendern und API‑Schlüsseln ohne geeignete Isolation. In vielen Fällen wird die lokale Web‑Schnittstelle öffentlich erreichbar gemacht oder ohne Authentifizierung betrieben, so dass Angreifer den Agenten über das Netzwerk steuern können.

Im Februar 2026 wurde zudem eine schwerwiegende Schwachstelle (CVE‑2026‑25253) gemeldet, die einen one‑click RCE‑Angriff ermöglichte. Dabei vertraute die Control‑UI der Abfrageparameter zu stark und übermittelte beim Verbindungsaufbau automatisch ein Gateway‑Token an eine beliebige Web‑Site; ein Klick auf einen präparierten Link reichte aus, um das Token an einen Angreifer zu senden. Der Angreifer konnte damit die Konfiguration ändern und beliebige Befehle auf dem Host ausführen. Die Ursache lag in fehlender Validierung des WebSocket‑Origins; so konnte eine bösartige Seite das Token abfangen, sich gegenüber dem Gateway authentifizieren und die Sandbox umgehen. Dieser Vorfall zeigt, dass existierende Sicherheitsfunktionen wie Sandboxen und Genehmigungsmechanismen die Gefahr nicht ausreichend eingrenzen.

Ein weiteres Problem ist das noch unzureichend geprüfte Ökosystem von „Skills“. Community‑Plugins können unbemerkt schädlichen Code oder Malware enthalten und weitreichende Berechtigungen anfordern. Sicherheitsforscher fanden Dutzende bösartige Skills auf dem inoffiziellen Marktplatz ClawdbHub.
...
Problem: Die erwähnte Schwachstelle (CVE‑2026‑25253) demonstriert, dass Tokens an das Web geleakt werden können. Die Control‑UI leitete das gatewayUrl aus der URL ohne Validierung an den WebSocket‑Verbindungsaufbau weiter; dadurch konnte ein Angreifer das Gateway‑Token abfangen. Weitere Untersuchungen zeigten, dass durch Cross‑Site‑WebSocket‑Hijacking selbst Gateways, die nur auf localhost hörten, kompromittiert werden konnten. Zusätzlich konnten Angreifer Konfigurationsparameter wie exec.approvals.set oder tools.exec.host ändern und so die Sandbox aushebeln.

Implikation: Wenn ein Angreifer das Gateway‑Token stiehlt, erhält er administrative Kontrolle über den Agenten. Er kann Sicherheitsmechanismen ausschalten, die Ausführung vom Container auf den Host umstellen und willkürliche Befehle ausführen. Bestehende LLM‑Sandboxen und Genehmigungssysteme schützen nicht vor dieser Klasse von Angriffen. Der Vorfall belegt, dass eine einfache Bindung an localhost nicht ausreicht, wenn der Browser des Nutzers als Brücke missbraucht werden kann.
...
Token‑Null‑Exposition (G1). Das LLM darf niemals Zugriff auf API‑Schlüssel, Tokens oder andere Geheimnisse erhalten. Geheimnisse verbleiben im Secrets‑Broker und werden nur als nicht wiederverwendbare Handles an Runner herausgegeben. So wird verhindert, dass ein kompromittiertes LLM Tokens exfiltriert oder in Prompts weiterleitet.

Standardmäßiges Verweigern (G2). Es sind keine Aktionen ohne explizite Genehmigung erlaubt. Alle Fähigkeiten sind zunächst deaktiviert; Nutzer müssen sie pro Agent, Workspace oder Job explizit freischalten.

Vertragsbasierte Ausführung (G3). Jedes Tool wird ausschließlich im Rahmen eines genehmigten Vertrags ausgeführt – entweder einmalig (One‑Shot) oder wiederverwendbar (für Jobs). Verträge definieren Parametergrenzen, Budgets, Datenhandhabung und Gültigkeitsdauer.

Isolierte Ausführung (G4). Die Agenten‑Engine darf keine Tools direkt ausführen. Werkzeuge werden in Runnern (WASM‑Sandboxen oder isolierten Containern) ausgeführt. Native Funktionen sind nur über Companion‑Services möglich, die per mTLS authentifiziert sind.

Vertrauenswürdige Bestätigungen (G5). Risikenbehaftete Aktionen (z. B. externe Sendungen, Dateischreibzugriffe) erfordern eine Bestätigung über die Control‑UI (paart mit Endgerät). Genehmigungen über Chat‑Nachrichten werden nicht akzeptiert.

Auditierbarkeit (G6). Alle Entscheidungen (Allow, Deny, Approval), alle Tool‑Aufrufe, Artefakterstellungen und ausgehenden Aktionen werden lückenlos in einer unveränderlichen Audit‑Log aufgezeichnet.
...
Angriffe auf die UI (Cross‑Site‑Hijacking). Tokens dürfen nicht im Browser gespeichert werden; mTLS und strikte Header‑Validierung sind Pflicht, um RCEs wie CVE‑2026‑25253 zu verhindern.
...
Gateway: Zentrale Schnittstelle für alle Eingaben (Messenger, E‑Mail, UI). Es verwaltet Sitzungen, Pairing und zeigt Genehmigungsanfragen an. Das Gateway hat keine Ausführungsberechtigung; es leitet lediglich Nachrichten und Entscheidungsergebnisse weiter.

Engine: Füttert das LLM, extrahiert Absichten, generiert Pläne und schlägt Tool‑Aufrufe vor. Sie hat keine Möglichkeit, selbst Tools auszuführen oder auf Secrets zuzugreifen.

Policy‑Engine: Bewertet vorgeschlagene Tool‑Aufrufe anhand von Verträgen, Fähigkeiten und Budgets. Ergebnis ist „Allow“, „Deny“ oder „Genehmigung erforderlich“. Datenwächter prüfen In‑ und Out‑Bound‑Daten auf sensible Inhalte.

Runner: Ausführungsumgebung für Tools. Jede Fähigkeit läuft in einer isolierten Sandbox (WASM oder Container). Runner nutzen den Secrets‑Broker, um API‑Aufrufe mit kurzen Handles durchzuführen.

Skill‑Registry: Repository für signierte WASM‑Skills und deren Metadaten. Hier werden auch Freischaltungen je Agent/Workspace verwaltet.

Secrets‑Broker: Speichert langfristige Geheimnisse (OAuth‑Refresh‑Tokens, API‑Schlüssel). Gibt pro Aufruf nur kurzlebige, parametergebundene Handles an Runner aus.

Scheduler: Führt wiederverwendbare Verträge zeitgesteuert oder ereignisgesteuert unter strenger Policy aus.

Audit‑Log: Unveränderliche Protokolle aller Aktivitäten, Entscheidungsvorgänge, Verträge, Genehmigungen und Artefakterstellungen.
...
Vertrag laden und prüfen: Ist er gültig, nicht abgelaufen und genehmigt?

Principal‑Abgleich: Stimmt der Principal (Session / Job) mit dem im Vertrag überein?

Parametervalidierung: Entspricht der Aufruf dem Binde‑Modus und den Constraints?

Budgetprüfung: Sind Limits für diesen Lauf noch nicht überschritten?

Risikoklasse: High‑Risk‑Tools erfordern interaktive Genehmigung.

Ergebnis protokollieren und Entscheidung zurückgeben (Allow / Deny / Approval).
...
Policy erteilt Erlaubnis: Bei erfolgreicher Prüfung generiert die Policy einen internen Entscheidungseintrag.

Runner fordert Handle an: Er ruft AcquireHandle(decision_id, tool_call_id) beim Broker auf.

Broker prüft: Ist die Entscheidung gültig? Stimmt der Runner‑Identität? Sind die Paramater gebunden?

Broker gibt Handle zurück: Der Handle ist nur für diesen einen Aufruf, das spezifische Werkzeug, den Parameter‑Hash und eine kurze TTL gültig.

Runner führt Aktion aus: Mit dem Handle ruft der Runner intern die API auf.

Handle verliert seine Gültigkeit: Er kann nicht wiederverwendet oder außerhalb des Runners exfiltriert werden.
...
Das Framework unterstützt Cron‑Jobs, Event‑Trigger (Webhook, Datenbank‑Änderungen etc.) und One‑Shot‑Timer. Kernelemente:
...
Idempotenz: Jede Side‑Effect‑Aktion hat einen Idempotenz‑Key (job_run_id + action_hash). Bei Wiederholungen (z. B. Retries) werden Duplikate verhindert.
...
Planung: Das LLM erstellt eine Liste der zu bearbeitenden Dateien, eine kurze Beschreibung der Änderungen und die Teststrategie.

Patch‑Erzeugung: Das Tool codegen.patch generiert einen Diff basierend auf den Plänen. Der Diff wird als Artefakt gespeichert. Parameterbeschränkungen (Max Dateianzahl, Max Diff‑Größe, keine Änderungen an .env oder Schlüsseldateien) sind Teil des Vertrags.

Validierung: Ein Runner führt Tests (pnpm test, pnpm lint) in einer isolierten Umgebung aus (kein Netzwerk). Testprotokolle werden als Artefakt abgelegt.

Review: Optional wird ein zweiter Agent (Reviewer) mit Lesezugriff auf den Patch‑Artefakt beauftragt, um Feedback zu geben.

Anwendung: Ein dritter Agent (Applier) darf den Patch nur anwenden (z. B. via git apply), wenn der Nutzer über die UI einen One‑Shot‑Vertrag genehmigt.

Zusammenfassung: Das LLM erstellt eine Kurzbeschreibung der Änderungen, die geänderten Dateien und Hinweise zum Testen.
