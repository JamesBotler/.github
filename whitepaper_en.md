...
Artificial intelligences acting as personal assistants or business agents are rapidly gaining popularity. Projects like OpenClaw (formerly ClawdBot or MoltBot) demonstrate how powerful such agents can be. They simultaneously connect to messengers, email systems, cloud services, git repositories and operating‑system tools. That convenience comes with severe security risks. Analyses show that OpenClaw requires wide‑ranging permissions – file system access, bash execution rights, API keys and network access – so complexity and the attack surface rise. Users often install the assistant with broad access to email, files, calendars and API keys without proper isolation. In many installations the local web interface is exposed publicly or operated without authentication, allowing attackers to control the agent over the network.

In February 2026, a high‑severity vulnerability (CVE‑2026‑25253) was reported that enabled a one‑click RCE attack: the control UI trusted a gatewayUrl query parameter and automatically sent the gateway token when establishing a WebSocket connection. Clicking a crafted link was enough to leak the token to an attacker, who could then modify the configuration and execute arbitrary commands. The root cause was missing validation of the WebSocket origin; thus a malicious page could capture the token, authenticate to the gateway and bypass the sandbox. This incident shows that existing security features such as sandboxes and approval mechanisms do not sufficiently mitigate such attacks.

Another problem is the unvetted ecosystem of “skills.” Community plugins can carry malware and request unrestricted permissions. Researchers found numerous malicious skills on the unofficial marketplace ClawdbHub.
...
Problem: The CVE‑2026‑25253 bug shows that tokens can be leaked to the web. The control UI forwarded the gatewayUrl from the query string directly into the WebSocket connection; thus an attacker could intercept the gateway token. Further analysis revealed that cross‑site WebSocket hijacking could compromise gateways that bound only to localhost. Attackers could disable confirmation requirements (exec.approvals.set) or run shell tools on the host by toggling tools.exec.host.

Implication: Stealing the gateway token grants administrative control over the agent. The attacker can turn off safety mechanisms, run commands on the host instead of the container and perform arbitrary actions. Existing sandbox and approval systems do not protect against this class of attack. Binding to localhost alone is insufficient because the user’s browser can be abused as a bridge.
...
Zero token exposure (G1). The LLM must never obtain access to API keys, tokens or other secrets. Secrets remain in the secrets broker and are released only as non‑reusable handles to runners. This prevents a compromised model from exfiltrating tokens.

Deny by default (G2). No actions are allowed without explicit approval. All capabilities are initially disabled; users must enable them per agent, workspace or job.

Contract‑based execution (G3). Each tool is executed only within an approved contract – either one‑shot or reusable. Contracts define parameter limits, budgets, data handling and validity.

Isolated execution (G4). The agent engine cannot execute tools directly. Tools run in runners (WASM sandboxes or isolated containers). Native functions are accessible only via companion services.

Trusted approvals (G5). Risky actions (e.g., external sends, file writes) require approval through the control UI (paired device). Approvals via chat messages are not accepted.

Auditability (G6). All decisions (allow, deny, approval), all tool calls, artifact creations and outgoing actions are logged immutably.
...
UI attacks (cross‑site hijacking). Tokens must not be stored in the browser; mTLS and strict header validation are required to prevent RCEs like CVE‑2026‑25253.
...
Gateway: The central interface for all inputs (messengers, email, UI). It manages sessions, pairing and displays approval requests. The gateway has no execution rights; it only forwards messages and decision results.

Engine: Feeds the LLM, extracts intent, generates plans and proposes tool calls. It cannot execute tools or access secrets.

Policy Engine: Evaluates proposed tool calls based on contracts, capabilities and budgets. Its output is “allow,” “deny” or “approval required.” Data guards inspect inbound and outbound data for sensitive content.

Runner: The execution environment for tools. Each capability runs in an isolated sandbox (WASM or container). Runners use the secrets broker to call APIs with short‑lived handles.

Skill Registry: The repository for signed WASM skills and their metadata. It also manages enablement per agent/workspace.

Secrets Broker: Stores long‑lived secrets (OAuth refresh tokens, API keys). Releases only short‑lived, parameter‑bound handles to runners.

Scheduler: Executes reusable contracts on schedules or in response to events under strict policy enforcement.

Audit Log: Immutable record of all activities, decision processes, contracts, approvals and artifacts.
...
Loads the contract and checks if it is valid, not expired and approved.

Ensures the principal (session / job) matches the contract.

Validates parameters according to binding mode and constraints.

Checks budgets: are limits for this run not yet exceeded?

Considers risk class: high‑risk tools require interactive approval.

Logs the result and returns allow/deny/approval.
...
Policy grants permission: On successful evaluation the policy generates an internal decision record.

Runner requests handle: It calls AcquireHandle(decision_id, tool_call_id) on the broker.

Broker verifies: Is the decision valid? Does the runner identity match? Are parameters bound?

Broker returns handle: The handle is valid only for this call, this tool, a specific parameter hash and a brief TTL.

Runner performs action: It uses the handle internally to call the API.

Handle expires: It cannot be reused or exfiltrated outside the runner.
...
Plan: The LLM generates a list of files to be modified, a short description of the changes and the test strategy.

Patch generation: The codegen.patch tool creates a diff based on the plan. The diff is stored as an artifact. Parameter restrictions (max file count, max diff size, no changes to .env or key files) are part of the contract.

Validation: A runner executes tests (pnpm test, pnpm lint) in an isolated environment (no network). Test logs are stored as an artifact.

Review: Optionally, a second agent (reviewer) with read‑only access to the patch artifact provides feedback.

Apply: A third agent (applier) may apply the patch (e.g. via git apply) only when the user approves a one‑shot contract in the control UI.

Summarise: The LLM creates a short summary of the changes, lists the files changed and provides testing instructions.
